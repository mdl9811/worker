...
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    // C6386 is suppressed because __crt_typefix(unsigned int*) annotation on 
    // _MarkAllocaS denotes that an unsigned int must fit at the returned value
    // after it is adjusted for the alloca marker, but it is permitted to 
    // _malloca / _malloca_crt an array with a type smaller than unsigned int.
    #define _malloca_crt(size)                                                                 \
        __pragma(warning(suppress: 6255 6386))                                                 \
        (_MallocaComputeSize(size) != 0                                                        \
...
extern "C++" {
        ...
        C const comparand) noexcept
    {
        ...
        inline __int32 __crt_interlocked_read_32(__int32 const volatile* target) noexcept
        {
            ...
            __int32 const result = __ldar32((volatile unsigned __int32*)target);
            return result;
            ...
            {
                #if defined _M_ARM64 || defined _M_ARM64EC
                __int64 const result = __ldar64((volatile unsigned __int64*)target);
                return result;
                #elif defined _M_X64
