...
MoveAbove32:
        ...
        cmp      rcx, rdx                                        ; i.e. src (rdx) < dst (rcx) < src + len (rdx+r8)
        cmovbe   r9, rcx
        cmp      rcx, r9
        jb       CopyDown                                        ; if it is copying the block requires using the special ("copy down") implementation   
...
;
        align    16
CopyDown:
        movups   xmm2, [rdx]             ; load lowest 16 bytes, which includes remainder 
        sub      rdx, rcx
...
;
        test     cl, 0Fh                 ; test if dest aligned
        jz       XmmMovLargeTest         ; go try 128-byte blocks
;
; Move alignment bytes.
;
XmmMovAlign:
        mov      r9, rcx                 ; save unaligned store address
        ...
        movups   [r9], xmm1              ; now safe to do unaligned store
        mov      r8, rcx                 ; easier to recalc length (r8) using rcx-rax ...
        sub      r8, rax                 ; ... than calc how much to subtract from it
...
XmmMovLargeInner:
        ...
        and      r8, 7Fh                 ; compute remaining byte count
...
XmmMovTrailing:
        and      r8, 0Fh                 ; compute remaining byte count
        jz       short XmmMovReturn      ; if z, no residual bytes to move

;
; Handle remainder bytes at the start.
;
        movups   [rax], xmm2             ; store lowest 16 bytes, which includes remainder
